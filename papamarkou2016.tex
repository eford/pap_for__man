...
of $\epsilon$ which yields a limiting acceptance rate of $0.574$ in high-dimensional parametric spaces 
...
To ensure that the adaptive Metropolis covariance estimator receives exact geometric updates regularly, a $\mod{a}$ scheduler
can be employed. If the $k$-th MCMC iteration is a multiple of a tuning constant $a$, then a SMMALA update is attempted, else
an AM proposal is made. Algorithm \ref{amsmmala} summarizes AMSMMALA with the help of pseudocode.
...
an expected number of $(1-p)/p=a$ AM steps before a first SMMALA proposal is made. This way, the $\mbox{Geometric}(1/(1+a))$ 
schedule is a stochastic analogue to the deterministic $\mod{a}$ schedule of Algorithm \ref{amsmmala}.

Other samplers with dual proposal mechanism, such as ALSMMALA, do not necessitate regular geometric steps, so it makes sense 
to exploit local geometric information mostly in the early transient phases of the simulated chain in order to save as much 
computational time as possible in late stationary phases. A $\mbox{Bernoulli(p(i))}$ distribution comes in handy, leaving
space for experimentation with the choice of schedule for $p(i)$. The probability $p(i)$ plays a role analogous to the
temperature in the cooling schedule of simulated annealing. This analogy brings up a prolific body of literature to open up 
possibilities for scheduling the cooling of probability $p(i)$ of geometric updates.
...
Computational efficiency involves two aspects of complexity. One is time required to explore state space effectively. The
other one is complexity in terms of computational cost of a single MCMC iteration. For instance, MALA takes
$\mathcal{O} (n_{\theta}^{1/3})$ time to explore a state space of size $n_{\theta}$, whereas the cost of a
single iteration costs $\mathcal{O} (n_{\theta})$.

Of course these complexities are not always known, esp for more complex algorithms. For instance, the average time SMMALA
takes to explore state space is not known theoretically, but its computational cost per iteration is
$\mathcal{O} (n_{\theta}^3)$. For this reason, there are alternative metrics that can be evaluated numerically. More
specifically, the effectiveness of a Monte Carlo algorithm in terms of time required to explore parameter space is quantified
using the effective sample size (ESS), whereas the computational complexity can be simply quantified in terms of CPU time.

To have an overall measure of computational efficiency, ESS and CPU time are combined by taking their ratio ESS/runtime.

Before proceeding any further, a couple of clarifications are made. Firstly, as elaborated in previous section, complexity 
of SMMALA is taken to be $\mathcal{O} (n_{\theta}^3)$.
Furthermore, it is emphasized that the implicit assumption is that the evaluation of log-target is also naively assumed to
be of same order. Otherwise, for expensive targets (ex requiring numerical solution of differential equations), the 
complexity is determined by the cost of log-target evaluation and can be of different order. Another way to put it, it is
assumed that the complexity of log-target is $\mathcal{O} (1)$.
