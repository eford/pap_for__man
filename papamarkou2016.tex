...
%% Lemma
%
%For sufficiently large number $n_m$ of MCMC iterations, the mean complexity of an algorithm $A$ with exponentially decaying
%schedule given by X is
%
%% Proposition
%
%Either of a $\mod{(a)}$ schedule $p(i)$ or a Geometric schedule with $p=a$ yield a mean complexity
%for $A$
%
%% Lemma
%
%ALSMMALA with exponentially decaying $p(i)$ has a mean complexity of
%
%% Lemma
%
%AMSMMALA has an average complexity of 
%
%The schedule plays a decisive role in the overall complexity. The decaying rate a gives a way of tuning the complexity.
%Obviously a determines the runtime vs frequency of geometric updates trade-off.
%
%Figure X shows the complexity of ALSMMALA and AMSMMALA as a function of decaying rate a and modulo factor a. In both cases,
%reducing the value of a increases frequency of geometric updates thus approximating the complexity of the expensive
%geometric proposal. Increasing the value of a reduces the amount of geometric computation, therefore reducing runtime
%towards the n and n cost of the respective MALA and AM proposals.

%givesestimator of the population mean is the number n effn_{eff} with the property that our estimator has the same 
%precision (or variance) as the estimator got by sampling n effn_{eff} independent individuals.
