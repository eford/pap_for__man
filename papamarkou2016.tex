...
\begin{lemma}
For a large number $n_m$ of iterations ($n_m\rightarrow\infty$), the mean complexity of an ALSMMALA sampler equipped with 
the exponentially decaying schedule defined by \eqref{exp_schedule_a} is
\begin{equation}
\label{eq:c_alsmmala_exp}
\mathcal{O}\left(
\dfrac{1-\exp{(-a)}}{a}n_{\theta}^3+
\left(1-\dfrac{1-\exp{(-a)}}{a}\right)n_{\theta}
\right).
\end{equation}
\end{lemma}
...
MALA are $\mathcal{O}(n_{\theta}^3)$ and $\mathcal{O}(n_{\theta})$.
\end{proof}

\begin{lemma}
The expected complexity of AMSMMALA, as presented in Algorithm \ref{amsmmala}, is
\begin{equation}
