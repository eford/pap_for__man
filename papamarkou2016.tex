...
computation of gradient and Hessian of order  $\mathcal{O}(n_\theta^2)$ or the inversion of the Hessian, so it becomes at best of order $\mathcal{O}(n_\theta^3)$.
% <- CHECK:  Shouldn't these be $\mathcal{O}(f(n_\theta)n_\theta^2)$   and $\mathcal{O}(f(n_\theta)n_\theta^2+n_\theta^3)$  
