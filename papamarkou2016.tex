...
proposal. Consequently, the cost of computations associated with the proposal kernel is of order $\mathcal{O}(3n_\theta^3)$ 
for each of these two samplers.

% MALA, as opposed to SMMALA and MMALA, relies on a constant preconditioning matrix G,
%therefore $G^{-1}$ and $\sqrt{G^{-1}}$ are evaluated once and cached at the beginning of the simulation avoiding the
%$\mathcal{O}(2n_\theta^3)$ cost.
%Once again, MALA avoids the $\mathcal{O}(n_\theta^3)$ penalty at proposal evaluations as the
...
% <- CHECK:  Shouldn't MMALA be (3+f(n_\theta))n_\theta^3 for consistency with next-to-last-paragraph of this section?
%
...
% <- CHECK:  Shouldn't these be 
%$\mathcal{O}((1+f(n_\theta)n_\theta^3+2n_\theta^{2.373})$ and %$\mathcal{O}(n_\thetaf(n_\theta)+n_\theta^3+2n_\theta^{2.373})$ time, respectively.
...
%<- CHECK   $\mathcal{O}((1+f(n_{\theta}))n_{\theta})$.

% CHECK If we agree on the above change, then can revise this paragraph, replacing the first three sentances with
% For many scientific applications, the complexity of evaluating the log-target will substantially exceed the complexity of the samplers.  
%
...
% CHECK:  If we agree on above changes, can move first two sentances earlier
...
% CHECK: That you agree with this correction
