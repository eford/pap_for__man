...
models, the cost of computations implicating the log-target is much higher than the cost of proposal-related calculations. 
In other words, if the log-target is of high complexity, then derivative computations supersede linear algebra calculations, 
...
\begin{lemma}
	For a log-target with complexity $\mathcal{O}(1)$, the expected complexity of AMSMMALA, as defined in Algorithm 
	\ref{amsmmala}, is
	\begin{equation}
...
\begin{figure}
	...
	\subfloat[Mean altitude per coarse grid cell]{\includegraphics[width=2.8in]{tree_altitude.pdf}} \\
	\subfloat[Mean slope per coarse grid cell]{\includegraphics[width=2.8in]{tree_gradient.pdf}} 
...
For example, consider the previous case of characterizing the masses and orbits of extrasolar planets.  \cite{rei_tam__sec} have implemented second-order variational equations into the REBOUND n-body integrator.  They confirm that the ratio of runtimes for the first and second-order variational equations for $N_{par}$ parameters scales as $(1+N_{par}+\frac{1}{2}N_{par}(N_{par}+1))$.  For a system with $N_{pl}$ planets, integrating just the standard model for one orbital period requires $\simeq~0.2 \mathrm{ms} \times N_{pl}(N_{pl}+1)/2$ on a single core of an Intel Xeon E5-1650 (3.2 GHz).  Since each planet requires 7 parameters, calculating the full second-order variational equations scales requires $\simeq~0.2 \mathrm{ms} N_{pl}(N_{pl}+1)(1+7N_{pl}+21N_{pl}(N_{pl}+1))$.  For example, an observational dataset of 5 years and a planetary system containing 4 planets including an innermost planet with an orbital period of 3.65 days would result in a the compute time for each model evaluation would be $\simeq~1$ second.  Evaluating the model and gradient would cost $\simeq~29$ seconds, while evaluating the model, gradient and full hessian would require $\simeq~435$ seconds.  For such problems, ALSMMALA can be expected accelerate computations by over an order of magnitude relative to SMMALA.
